...
public class SoundManager implements Runnable {
		...
		while (!iskilled) {
			...
			try {
				if (this.currentplayer.isComplete() && !iskilled) {
					...
					this.setBackgroundMusic(currentsong);
				}
	...
	public void setBackgroundMusic(Song song) {
		if (!iskilled) {
			...
			try {
				...
				InputStream is = classloader.getResourceAsStream(song.getfilepath());
				this.currentplayer = new Player(is);
			} catch (Exception e) {
				if (song != null)
	...
	/**
	 * Closes the SoundManager and release the resources. Also stop the thread.
	 */
	public void kill() {
		this.iskilled = true;
		currentplayer.close();
